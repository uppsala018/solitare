...
export default function Leaderboard({ entries, currentUserId, prizePool, compact }: LeaderboardProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{
                  background: isSelf ? 'rgba(245,200,66,0.3)' : 'rgba(255,255,255,0.12)',
                  color: isSelf ? '#f5c842' : 'white',
                }}
              >
      ...
      {/* User not in list but exists */}
      {currentUserId && userRank === 0 && (
        <div
          className="flex items-center gap-2 px-3 py-2 rounded-xl mt-1"
          style={{ background: 'rgba(245,200,66,0.1)', border: '1px dashed rgba(245,200,66,0.3)' }}
        >
          <Crown size={14} style={{ color: '#f5c842' }} />
          <span className="text-gold text-xs">You haven't scored yet</span>
        </div>
      )}
    </div>
  );
